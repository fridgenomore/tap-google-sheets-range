...
class GoogleClient: # pylint: disable=too-many-instance-attributes
    ...
    def get_access_token(self):
        ...
        if self.__sa_keyfile:
            with open(self.__sa_keyfile) as f:
                try:
                    f_data = json.load(f)
                    sa_email = f_data['client_email']
                except:
                    LOGGER.error('Failed to extract client_email from {}'.format(self.__sa_keyfile))
                    raise
            token = self.generate_jwt(self.__sa_keyfile, sa_email)
            data = {
                'grant_type': 'urn:ietf:params:oauth:grant-type:jwt-bearer',
                'assertion': token.decode('ascii'),
            }
        else:
            data = {
    ...
    def get(self, path, api, **kwargs):
        LOGGER.info("Get method was called. Path: {} Api: {} Args: {}".format(path, api, kwargs))
        return self.request(method='GET', path=path, api=api, **kwargs)

    def post(self, path, api, **kwargs):
        LOGGER.info("Post method was called. Path: {} Api: {} Args: {}".format(path, api, kwargs))
